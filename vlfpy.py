...
def phase_function(data, t, T, fs):
    ...
    data_nrz = data.copy() 
    data_nrz[data_nrz == 0] = -1 # NRZ encoding
...
def generate_msk(N, p, T, fc, fs, offset=0, phase_offset=0, amplitude=1):
    ...
    padding =  np.random.randint(2,5) # padding to ensure random starting phase
    data = generate_data(N +padding, p) 
    #data = generate_data(N, p) # uncomment this line to remove padding
    t = np.linspace(0, (len(data))*T, int(len(data)*T*fs), endpoint=False)
    phase = phase_function(data, t, T, fs) # phase function
    phase = np.roll(phase, int(fs*offset)) # timing offset


    phase = phase[(padding-1)*int(fs*T):-int(fs*T)] # remove padding
    data = data[padding-1:-1]
   
    t = np.linspace(0, (len(data))*T, int((len(data))*T*fs), endpoint=False)

    s = amplitude * np.cos(2*np.pi*fc*(t-offset) + phase + phase_offset) # MSK signal with phase offset
  
    return s, t, data, phase

def generate_msk_data(data_in, T, fc, fs, offset=0, phase_offset=0, amplitude=1):
    # data1: input data
    # T: symbol duration
    # fc: carrier frequency
    # fs: sampling frequency
    ...
    data_extended = np.concatenate((np.ones(1), np.zeros(1),data, np.zeros(1), np.ones(1))) # extend data to avoid edge effects
    ...
    phase = phase[2*int(fs*T):-2*int(fs*T)] # remove padding
    t = np.linspace(0, (len(data))*T, int(len(data)*T*fs), endpoint=False)
...
def amplitude_estimation(r):
    ...
    a_hat = (a_hat + np.mean(np.abs(r.imag)*np.pi))/2 # average of real and imaginary part
...
def viterbi_decoder(s, N):
    ...
    path1 = np.array([-1]) # initial path
    path2 = np.array([1])

    pm1 = viterbi_metric(s[:N], [], path1, phase_start) # initial metrics
    pm2 = viterbi_metric(s[:N], [], path2, phase_start)
    
    for i in range(1,len(s)//N):

        # calculate metrics for new symbols
    ...
    path1[path1 == -1] = 0 # convert to binary
    path2[path2 == -1] = 0
